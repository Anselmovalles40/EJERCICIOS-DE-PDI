...
try:
    ...
    # Estimación de la PSF (Función de degradación)
    psf_estimada = np.ones_like(psf)  # Estimación trivial de la PSF para este ejemplo

    # Restauración utilizando deconvolución ciega (Wiener)
    restored = restoration.wiener(degraded, psf_estimada, 0.1)
    ...
    plt.title('Imagen Restaurada (Filtrado Inverso)')
    plt.axis('off')
